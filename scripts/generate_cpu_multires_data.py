"""
Multi-resolution training data generator (CPU, no JAX required).

Generates 3000 samples at 16×16, 32×32, 64×64, 128×128 from the *same*
fine-scale (128×128) permeability field using the existing SciPy solver.
...
Single process:
    python scripts/generate_cpu_multires_data.py

Multi-process (e.g. 4 CPU cores):
    python scripts/generate_cpu_multires_data.py --shard 0 --num-shards 4 &
    python scripts/generate_cpu_multires_data.py --shard 1 --num-shards 4 &
    python scripts/generate_cpu_multires_data.py --shard 2 --num-shards 4 &
    python scripts/generate_cpu_multires_data.py --shard 3 --num-shards 4 &
    wait
    python scripts/generate_cpu_multires_data.py --merge --num-shards 4

Background:
    nohup python scripts/generate_cpu_multires_data.py > generate.log 2>&1 &
"""
...
def run_scipy_simulation(
    ...
    wy, wx = remap_well(well_y, well_x, NX_FINE, NY_FINE, nx, ny)
    ...
    pressure = np.full(nx * ny, initial_pressure_Pa, dtype=np.float64)
    results = [pressure.reshape(ny, nx).copy()]
    current_time = 0.0
    next_snap = 1
    ...
    while current_time < TOTAL_TIME and next_snap < N_TIME_SLICES:
        ...
        A, b = discretizer.discretize_single_phase(actual_dt, pressure, well_manager)
        pressure = solver.solve(A, b)

        current_time += actual_dt
        while next_snap < N_TIME_SLICES and current_time >= snap_times[next_snap] - 1.0:
            results.append(pressure.reshape(ny, nx).copy())
            next_snap += 1
...
def generate_shard(shard: int, num_shards: int, output_root: str):
    ...
    logger.info(f"Generating {n_shard} samples (indices {sample_indices[0]}..{sample_indices[-1]}) "
                f"at resolutions {RESOLUTIONS} [CPU, scipy direct solver]")
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-resolution training data generator (CPU, no JAX)")
    parser.add_argument("--shard", type=int, default=None,
                        help="Shard index (0-based). If omitted, runs all on one process.")
